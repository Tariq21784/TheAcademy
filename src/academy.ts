...
function splitIntoSections(text: string) {
  ...
  if (sections.length === 0) {
    ...
    sections.push({ id: newId(), title: 'Study unit', body, keywords: buildKeywords(body) });
  }
...
export function evaluateAnswer(answer: string, quizItem: QuizItem): AnswerResult {
  const clean = answer.toLowerCase().trim();
  const key = quizItem.answerKey.toLowerCase();
  const includesKey = key.length > 0 && clean.includes(key);
